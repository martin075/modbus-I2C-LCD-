//lcdpcf8574 lib copyright (c) Davide Gironi, 2013
// DS18B20 - +-0,5°C
// BMP180 - +-1°C, -4 +2 hPa
...
// 0010 0 A2 A1 A0 for PCF8574/PCF8574T (0x20-0x27)
// 0011 1 A2 A1 A0 for PCF8574A (0x38 - 0x3F)
#include "bmp085.h"					//i2c addr 0x77
#include "ds18b20.h"				
#include "yaMBSiavr.h"
...
volatile uint16_t holdingRegisters[14]= {40,41,42,43,44,45,47,48,0,0,29,28,15,0}; // default values for example
//volatile int inputRegisters[4];
//volatile int holdingRegisters[14]= {40,41,42,43,44,45,47,48,0,0,29,28,10,0}; // default values for example
...
int main(void)
{
	...
	DDRB &= ~(1 << PB0);   // button for turn LED backlight ON, INPUT
    ...
    lcd_gotoxy( 2, 0);	//column stlpec, row  riadok
	 lcd_puts_P( " modbus temp 0.2");
	sprintf( CharBuffer, "%i,8N1",BAUD);
	lcd_gotoxy( 0, 1);
	lcd_puts(CharBuffer);
	sprintf( CharBuffer, "modbus adr:%X",clientAddress);
	lcd_gotoxy( 4, 2);
	lcd_puts(CharBuffer);
	...
	if(chyba4==0)
       ...
       for(i=0; i<24; i++)
       {
		...
		// set up bmp085board
		bmp085_init_w(); 	//before was initialize by LCD init
...
//------------main loop-------------------------------------
    ...
    while(1) {
		...
		//----------------------------------------------------	
			...
			lcd_gotoxy( 3, 0);
			if((CurrentTemp1<0) && (cele == 0)) sprintf( CharBuffer, "-%i.%i",cele,desatiny);
			else sprintf( CharBuffer, "%+i.%i",cele,desatiny);
		...
		//----------------------------------------------------	
			...
			if((CurrentTemp2 < 0) && (cele == 0)) sprintf( CharBuffer, "-%i.%i",cele,desatiny);
			else sprintf( CharBuffer, "%+i.%i",cele,desatiny);
		...
		//----------------------------------------------------	
			...
			if((CurrentTemp3 < 0) && (cele == 0)) sprintf( CharBuffer, "-%i.%i",cele,desatiny);
			else sprintf( CharBuffer, "%+i.%i",cele,desatiny);
		...
		//----------------------------------------------------	
			...
			if((CurrentTemp4 < 0) && (cele == 0)) sprintf( CharBuffer, "-%i.%i",cele,desatiny);
			else sprintf( CharBuffer, "%+i.%i",cele,desatiny);
			...
			//_delay_ms(100);
		 //----------------------------------------------------			
		 	if((PINB & (1<< PB0))==0) 	//reading input pin PB0
				{led = 0;
				lcd_led(led);	//set led backlight ON
    			}
		 //----------------------------------------------------
			if(chyba5==0)
			{
			...
			if((CurrentTemp5 < 0) && (cele == 0)) sprintf( CharBuffer, "-%i.%i",cele,desatiny);
			else sprintf( CharBuffer, "%+i.%i",cele,desatiny);
		 ...
		 //----------------------------------------------------			
			...
			if((CurrentTemp6 < 0) && (cele == 0)) sprintf( CharBuffer, "-%i.%i",cele,desatiny);
			else sprintf( CharBuffer, "%+i.%i",cele,desatiny);
		...
		press++; // increment press - more delay for measurement of atmospheric pressure
			...
			PORTB ^= (1 << PB5); // set toggle
    		_delay_ms(500);
			led = 1;lcd_led(led); // set backlight OFF
